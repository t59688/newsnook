...
export function clsSignedListUrl(options?: { rn?: number; lastTime?: number }): string {
  const params: Record<string, string | number> = {
    app: 'CailianpressWeb', last_time: options?.lastTime ?? 0, os: 'web', refresh_type: 1,
    rn: options?.rn ?? 20, sv: '8.7.9',
  }
...
export function offsetPageRequest(source: NewsSource, page: number): OffsetPageRequest {
  ...
  if (source.kind === 'netease') return { url: neteasePageUrl(source, safePage) }
  if (source.kind === 'zhihu-main') {
    const url = new URL(source.url)
    url.searchParams.set('offset', String(safePage * 20))
    return { url: url.href }
  }
  if (source.kind === 'wordpress') {
    const url = new URL(source.url); url.searchParams.set('page', String(safePage + 1)); return { url: url.href }
  }
  if (source.kind === 'latepost') return { url: source.url, requestForm: { ...(source.requestForm ?? {}), page: safePage + 1 } }
  if (source.kind === 'cls') return { url: clsSignedListUrl({ rn: 20, lastTime: 0 }) }
  if (source.kind === 'eastmoney-news') {
    const url = new URL(source.url); url.searchParams.set('page_index', String(safePage + 1)); url.searchParams.set('req_trace', String(Date.now())); return { url: url.href }
  }
  if (source.kind === 'eastmoney-kx') {
    return { url: source.url.replace(/(_ajaxResult_\d+_)(\d+)(_\.html(?:\?.*)?)$/i, `$1${safePage + 1}$3`) }
  }
  ...
  if (source.kind === 'web-catalog') {
    if (source.frameworkHint) return { url: frameworkPageUrl(source.url, safePage, source.frameworkHint.paginationPattern) }
...
export function pagingStrategyOf(source: NewsSource): PagingStrategy {
  ...
  if (source.kind === 'zhihu-main') return 'upstream-offset'
