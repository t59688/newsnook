...
function unwrapFeedTarget(entry: unknown): UnknownRecord | undefined {
  ...
  if (content) {
    const nestedObject = record(content.object)
    if (nestedObject) return nestedObject
    const nestedTarget = record(content.target)
...
function titleFor(type: ZhihuContentType, target: UnknownRecord): string {
  ...
  const author = record(target.author)
...
function summaryFor(target: UnknownRecord, title: string): string {
  const raw =
    text(target.excerpt) ||
    text(target.description) ||
    text(target.detail) ||
    text(target.content) ||
    text(record(target.author)?.headline)
  const summary = stripHtml(raw)
  return cleanSummaryText(summary, title).slice(0, 220)
}
...
function imageFor(target: UnknownRecord): string | undefined {
  const thumbnail = record(target.thumbnail)
  const cover = record(target.cover)
  const rawContent = text(target.content)
  return safeImage(
    target.thumbnail,
    thumbnail?.url,
    thumbnail?.original,
    ...
    target.coverUrl,
    cover?.url,
    firstImage(rawContent),
  )
}

export function zhihuTargetToArticle(
  source: NewsSource,
  rawTarget: unknown,
  fetchedAt = Date.now(),
): Article | undefined {
  const target = unwrapFeedTarget(rawTarget)
  ...
  const title = stripHtml(titleFor(type, target))
  if (!title) return undefined
  const url = articleUrl(type, id, parentId)
  const published = publishedAtFor(target, fetchedAt)

  return {
    id: feedArticleId(source.id, url),
    title,
    summary: summaryFor(target, title),
    image: imageFor(target),
    publishedAt: published.value,
    hasRealDate: published.real,
    sourceId: source.id,
...
function pagingOf(payload: UnknownRecord): ZhihuPaging | undefined {
  ...
  return {
    isEnd: typeof paging.is_end === 'boolean' ? paging.is_end : typeof paging.isEnd === 'boolean' ? paging.isEnd : undefined,
    next: text(paging.next) || undefined,
    previous: text(paging.previous) || undefined,
  }
}

function payloadEntries(payload: UnknownRecord): unknown[] {
  const direct = array(payload.data)
...
export function normalizeZhihuList(
  source: NewsSource,
  payload: unknown,
  fetchedAt = Date.now(),
): ZhihuListResult {
...
export function normalizeZhihuListJson(
  source: NewsSource,
  payload: string,
  fetchedAt = Date.now(),
): ZhihuListResult {
  try {
    return normalizeZhihuList(source, JSON.parse(payload) as unknown, fetchedAt)
  } catch {
    return { articles: [] }
  }
}
