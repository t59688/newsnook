...
- **阅读位置记忆 + 系统分享**：长文重开回到上次读到的位置（分页模式同样适用）；阅读器「⋯」可把标题与原文链接交给系统分享面板
...
点击条目后，在应用内渲染适合手机阅读的正文，包含标题、来源、时间、图片、表格等常见结构。顶栏「⋯」收纳了分享文章、复制链接、在浏览器核对原文与重新抽取正文。长文中途退出后再打开会回到上次读到的位置，改过字号也按比例折算。
