...
本地优先的 Android 新闻阅读客户端：无账号、无后端、无云端推荐。订阅源由你配置，列表与正文由客户端直连上游获取，并在应用内阅读。
...
- **无云端推荐、无广告**：只展示你启用的源与类别；可选的「本地推荐」仅在本机按已读习惯重排，不做热度或云端强推（见 [docs/local-recommend.md](./docs/local-recommend.md)）
